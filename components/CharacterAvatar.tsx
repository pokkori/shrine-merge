...
const EMOTION_CONFIG = {
  normal: { eyeScaleY: 1, mouthPath: "M 35 62 Q 50 68 65 62", eyebrowOffset: 0, blushOpacity: 0, bodyAnimation: "avatar-idle", color: "#FF69B4" },
  happy: { eyeScaleY: 0.3, mouthPath: "M 32 60 Q 50 75 68 60", eyebrowOffset: -3, blushOpacity: 0.7, bodyAnimation: "avatar-bounce", color: "#FF85C2" },
  sad: { eyeScaleY: 1, mouthPath: "M 35 68 Q 50 60 65 68", eyebrowOffset: 4, blushOpacity: 0, bodyAnimation: "avatar-droop", color: "#C08FBF" },
  surprised: { eyeScaleY: 1.3, mouthPath: "M 44 64 Q 50 72 56 64", eyebrowOffset: -6, blushOpacity: 0.3, bodyAnimation: "avatar-shake", color: "#FF69B4" },
} as const;

export default function CharacterAvatar({ emotion = "normal", message, size = 120, className = "", colorOverride }: CharacterAvatarProps) {
  const config = EMOTION_CONFIG[emotion];
  const fillColor = colorOverride ?? config.color;
    ...
    <div className={`relative inline-block ${className}`} style={{ width: size, height: size + (message ? 60 : 0) }}>
      {message && (
        <div className="absolute left-1/2 -translate-x-1/2 top-0 z-10" style={{ animation: "bubble-appear 0.3s cubic-bezier(0.34,1.56,0.64,1) both" }}>
          <div className="relative px-3 py-2 text-sm font-bold text-gray-800 rounded-2xl shadow-lg" style={{ background: "rgba(255,255,255,0.92)", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)", border: "1.5px solid rgba(255,200,200,0.6)", maxWidth: 180, whiteSpace: "nowrap" }}>
            {message}
      ...
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2" style={{ animation: `${config.bodyAnimation} 2s ease-in-out infinite` }}>
        <svg width={size} height={size} viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg" aria-label={`桜キャラクターアバター（${emotion}）`} role="img">
          <ellipse cx="50" cy="95" rx="22" ry="5" fill="rgba(0,0,0,0.12)" style={{ animation: "shadow-pulse 2s ease-in-out infinite" }} />
          <circle cx="50" cy="55" r="28" fill={fillColor} />
          <ellipse cx="44" cy="48" rx="10" ry="7" fill="rgba(255,255,255,0.2)" />
          <circle cx="50" cy="30" r="22" fill={fillColor} />
          <ellipse cx="43" cy="22" rx="8" ry="6" fill="rgba(255,255,255,0.25)" />
          <ellipse cx="30" cy="24" rx="7" ry="9" fill={fillColor} style={{ transform: "rotate(-15deg)", transformOrigin: "30px 24px" }} />
          <ellipse cx="30" cy="24" rx="4" ry="6" fill="rgba(255,255,255,0.4)" style={{ transform: "rotate(-15deg)", transformOrigin: "30px 24px" }} />
          <ellipse cx="70" cy="24" rx="7" ry="9" fill={fillColor} style={{ transform: "rotate(15deg)", transformOrigin: "70px 24px" }} />
          <ellipse cx="70" cy="24" rx="4" ry="6" fill="rgba(255,255,255,0.4)" style={{ transform: "rotate(15deg)", transformOrigin: "70px 24px" }} />
          <path d={`M 33 ${22 + config.eyebrowOffset} Q 38 ${19 + config.eyebrowOffset} 43 ${22 + config.eyebrowOffset}`} stroke="rgba(120,40,80,0.6)" strokeWidth="2" strokeLinecap="round" fill="none" />
          <path d={`M 57 ${22 + config.eyebrowOffset} Q 62 ${19 + config.eyebrowOffset} 67 ${22 + config.eyebrowOffset}`} stroke="rgba(120,40,80,0.6)" strokeWidth="2" strokeLinecap="round" fill="none" />
          <g style={{ transform: `scaleY(${eyeScaleY})`, transformOrigin: "38px 28px", transition: "transform 0.08s ease" }}>
            <ellipse cx="38" cy="28" rx="5" ry="5" fill="#3D1828" />
          ...
          <g style={{ transform: `scaleY(${eyeScaleY})`, transformOrigin: "62px 28px", transition: "transform 0.08s ease" }}>
            <ellipse cx="62" cy="28" rx="5" ry="5" fill="#3D1828" />
            <ellipse cx="60" cy="26" rx="1.5" ry="1.5" fill="white" />
          </g>
          <ellipse cx="28" cy="34" rx="6" ry="4" fill="rgba(255,100,150,0.5)" opacity={config.blushOpacity} />
          <ellipse cx="72" cy="34" rx="6" ry="4" fill="rgba(255,100,150,0.5)" opacity={config.blushOpacity} />
          <ellipse cx="50" cy="33" rx="2" ry="1.5" fill="rgba(120,40,80,0.35)" />
          <path d={config.mouthPath} stroke="rgba(120,40,80,0.6)" strokeWidth="2.5" strokeLinecap="round" fill="none" style={{ transition: "d 0.4s cubic-bezier(0.34,1.56,0.64,1)" }} />
          <ellipse cx="22" cy="60" rx="7" ry="6" fill={fillColor} />
          <ellipse cx="78" cy="60" rx="7" ry="6" fill={fillColor} />
          <ellipse cx="40" cy="84" rx="8" ry="6" fill={fillColor} />
          <ellipse cx="60" cy="84" rx="8" ry="6" fill={fillColor} />
        </svg>
      </div>
      <style>{`
